...
async function gerarEPartilharImagem(htmlParaImagem, nomeFicheiro) {
    ...
    if (!containerPartilha || !conteudoPartilha) return;
    ...
    try {
        ...
        canvas.toBlob(async (blob) => {
            if (navigator.share && navigator.canShare({ files: [new File([blob], nomeFicheiro, { type: 'image/png' })] })) {
                await navigator.share({ files: [new File([blob], nomeFicheiro, { type: 'image/png' })] });
            } else {
...
const pesosEquivalentes = { Bifanas: 80, Entremeadas: 70, Hambúrgueres: 110, Salsichas: 60 };
const refeicoesDB = [
...
function sugerirJantar(surpresa = false, reRolagem = false) {
    ...
    if (reRolagem && ultimosFiltrosSugestao) {
        ...
        querLight = ultimosFiltrosSugestao.light;
        ...
        querLight = document.getElementById('sugestao-light').checked;
        ingredientesSelecionados = surpresa ? [] : Array.from(document.querySelectorAll('input[name="ingrediente"]:checked')).map(cb => cb.value);
        ultimosFiltrosSugestao = { rapida: querRapida, crianca: paraCriancas, light: querLight, ingredientes: ingredientesSelecionados };
    }
    ...
    if (querLight) {
        poolFinal = poolFinal.filter(r => r.light === true);
    }
